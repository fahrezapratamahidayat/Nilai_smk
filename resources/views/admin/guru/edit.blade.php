                ...
                <form action="{{ route('admin.guru.update', $guru->id) }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control @error('alamat') is-invalid @enderror"
                                  id="alamat" name="alamat" rows="3" required>{{ old('alamat', $guru->alamat) }}</textarea>
                        @error('alamat')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        <label for="kelas_ajar" class="form-label">Kelas Ajar</label>
                        <select class="form-select @error('kelas_ajar') is-invalid @enderror"
                                id="kelas_ajar" name="kelas_ajar" required>
                            <option value="">Pilih Kelas</option>
                            <option value="10" {{ old('kelas_ajar', $guru->kelas_ajar) == '10' ? 'selected' : '' }}>Kelas 10</option>
                            <option value="11" {{ old('kelas_ajar', $guru->kelas_ajar) == '11' ? 'selected' : '' }}>Kelas 11</option>
                            <option value="12" {{ old('kelas_ajar', $guru->kelas_ajar) == '12' ? 'selected' : '' }}>Kelas 12</option>
                        </select>
                        @error('kelas_ajar')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('status_guru') is-invalid @enderror"
                            ...
                            <option value="wali_kelas" {{ old('status_guru', $guru->status_guru) == 'wali_kelas' ? 'selected' : '' }}>Wali Kelas</option>
                            <option value="guru_mapel" {{ old('status_guru', $guru->status_guru) == 'guru_mapel' ? 'selected' : '' }}>Guru Mata Pelajaran</option>
                        </select>
                        @error('status_guru')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
